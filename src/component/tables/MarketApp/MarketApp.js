...
import { allUsers, blockStatusUser, blockUser } from "../../../redux/slice/userSlice";
...
import { getMarketDataAll } from "../../../redux/slice/marketAppSlice";
import MarketAppDialog from "./MarketAppDialog";
...
export const MarketApp = () => {
  ...
  const { marketAppData, total } = useSelector((state) => state?.marketApp);
  ...
  useEffect(() => {
    ...
    dispatch(getMarketDataAll(payload));
  }, [dispatch, page, rowsPerPage]);

  useEffect(() => {
    setData(marketAppData);
  }, [marketAppData]);
  ...
  const userTable = [
    ...
    },
    {
      Header: "Balance",
      Cell: ({ row }) => (
        <span className="text-capitalize" style={{ color: "red", fontWeight: "bold" }}>
          {row?.diamond ? row?.diamond : "-"}
        </span>
      ),
    },
    {
  ...
  const handleInfo = (id) => {
    // if (id === "65d9d2437523d4dbb9a51a25") return permissionError();
    navigate("/admin/marketapp/marketappview", {
      state: {
        id,
      },
    });
  ...
  const handleProfile = (row, type) => {
    // if (row?._id === "65d9d2437523d4dbb9a51a25") return permissionError();
  ...
  const handleStatus = (row) => {
    ...
    // if (id === "65d9d2437523d4dbb9a51a25") return permissionError();
    const payload = {
      status: row?.isBlock === true ? false : true,
      userId: row?._id
    }
    dispatch(blockStatusUser(payload));
  };
  ...
  return (
    <div className="mainCategory">
      ...
      {dialogue && dialogueType === "marketAppCreate" && (
        <MarketAppDialog setData={setData} data={data} />
      )}
    </div>
  );
};
