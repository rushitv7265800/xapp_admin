...
const ContentTable = () => {
  ...
  const handleClipBoard = (row) => {
    ...
    textField.remove()
    Success("Content Copy Successfull")
  }
  ...
  const categoryTable = [
    ...
    },
    {
      Header: "Content",
      Cell: ({ row }) => (
        <span style={{ textAlign: "left", whiteSpace: "pre-wrap" }}>{row?.content}</span>
      ),
    },
    {
      Header: "Date",
...

    {
      ...
      Cell: ({ row }) => (
        <span className="d-flex justify-content-center">
          <button
            className="py-1 me-2 copytext "
            style={{ backgroundColor: "rgb(254, 240, 191)", borderRadius: "8px" }}
            onClick={() =>
              handleClipBoard(row)
            }
          >
            <i class="fa-solid fa-copy" style={{ width: "26px", height: "26px", color: "#efbb00" }}></i>
          </button>
          <button
            className="py-1 me-2"
  ...
  return (
    <div className="mainCategory contentPage">
      <Title name="Content" />
      <Button
