...
import {
  ...
  getdepositPayment,
  isActiveDepositPayment,
} from "../../../redux/slice/depositmethodSlice";
import male from "../../../assets/images/male.png";
import DepositmethodDialog from "./DepositmethodDialog";

const Depositmethod = () => {
  const depositmethodData = useSelector((state) => state.depositmethod.depositmethodData);
  ...
  const hadnleIsActive = (row) => {
    const payload = {
      id: row?._id,
      data: { isActive: row?.isActive === true ? false : true }
    }
    dispatch(isActiveDepositPayment(payload));
  }
  ...
  const handleDelete = (id) => {
    ...
    data
      .then((logouts) => {
        ...
        console.log("yes", yes);
        if (yes) {
          dispatch(deleteDepositPayment(id));
        }
      })
  ...
  return (
    <div className="mainCategory">
      ...
      {dialogue && dialogueType === "banner" && (
        <DepositmethodDialog setData={setData} data={data} />
      )}
    </div>
  );
};
