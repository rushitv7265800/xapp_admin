...
export const Expert = () => {
  ...
  const handleInfo = (row) => {
    dispatch(openDialog({ type: "withdrawInfo", data: row }));
  };
  ...
  const expertTable = [
    ...
    },
    {
      Header: "with. Amount",
      body: "age",
      Cell: ({ row }) => <span>{row?.diamond ? row?.diamond : "0"}</span>,
      sorting: { type: "client" },
    },
    {
      Header: "Date",
      Cell: ({ row }) => <span className="text-capitalize text-nowrap">{row?.date ? row?.date : "-"}</span>,
    },
    // {
    //   Header: "Commission",
    //   body: "commission",
